# Latika-Jadhav-TPM
Technical Project Manager Portfolio 


# 👋 Hi, I'm Latika Agiwale

🚀 Technical Product Manager | AI & GenAI Enthusiast | .NET Full Stack

---

## 💡 About Me

PMP-certified Technical Product Manager with 11+ years of experience delivering enterprise products across AI, cybersecurity, and ERP platforms.

✔️ Expertise in Agile, Product Strategy, and AI-driven solutions
✔️ Strong technical foundation in .NET, APIs, and system design
✔️ Passionate about building scalable, user-centric products

---

## 🔥 Featured Projects

### 🤖 GenAI Penetration Testing Automation

👉 [View Project](./genai-pt-automation)

* Automated manual security testing using LLMs
* Reduced cycle time & improved coverage
* Integrated with CI/CD pipelines

---

### 📊 Agile Metrics Dashboard

👉 [View Project](./agile-metrics-dashboard)

* Sprint velocity, burndown, and release tracking
* Leadership-level KPI visualization

---

### 🧠 Product Case Studies

👉 [View Project](./product-case-studies)

* Real-world enterprise product decisions
* Architecture + trade-offs + KPIs

---

### ⚙️ API Design Playbook

👉 [View Project](./api-design-playbook)

* Scalable API architecture
* Security, versioning, and governance

---

### 🤖 TPM AI Assistant

👉 [View Project](./tpm-ai-agent)

* AI-powered backlog & sprint generator
* Converts requirements → epics → stories

---

## 🛠️ Tech Stack

* AI/ML: OpenAI, Prompt Engineering
* Backend: .NET, REST APIs
* Frontend: Angular, React
* Tools: JIRA, Jile, Confluence
* Cloud: Azure

---

## 📫 Contact Me

📧 Email: [latikajadhav17@gmail.com](mailto:latikajadhav17@gmail.com)
💼 LinkedIn:(https://www.linkedin.com/in/latika-jadhav-33a07354)
Phone Number : 9029836465

---

⭐ If you like my work, feel free to connect!
